...
interface ProfessionalIndustryDetailLayoutProps {
  ...
  industry: {
    ...
    personas?: RelatedPersona[];
    case_studies?: { id: string }[];
  };
}
...
export default function ProfessionalIndustryDetailLayout({
  ...
  const algorithms = (industry.algorithms ?? []).filter(a => a.slug !== 'not-applicable');
  const personas = (industry.personas ?? []).filter(p => p.slug !== 'not-applicable');
  const caseStudyCount = (industry.case_studies ?? []).length;
          ...
          <div className="flex flex-wrap gap-6 text-sm">
            ...
            {algorithms.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-foreground font-medium">
                  {algorithms.length} Related {algorithms.length === 1 ? 'Algorithm' : 'Algorithms'}
                </span>
              </div>
            )}
            ...
            {personas.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-foreground font-medium">
                  {personas.length} Target {personas.length === 1 ? 'Role' : 'Roles'}
                </span>
              </div>
            )}
          </div>
