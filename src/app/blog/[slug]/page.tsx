...
export const generateStaticParams = generateStaticParamsFor('blog-posts');
...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  const title = blogPost.title as string;
  const description = (blogPost.description as string | null) || 'Blog post from OpenQase';

  return {
    title: `${title} | OpenQase Blog`,
    description,
    alternates: {
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const processedContent = blogPost.content ? await processMarkdown(blogPost.content as string) : '';

  // Extract related blog posts from flattened relationships, filter out those without slugs
  const relatedPosts = ((blogPost.related_posts as Record<string, unknown>[] | undefined) ?? [])
    ...
    <>
      ...
      <AutoSchema
        ...
        breadcrumbs={[
          { name: 'Home', url: '/' },
          { name: 'Blog', url: '/blog' },
          { name: blogPost.title as string, url: `/blog/${blogPost.slug as string}` }
        ]}
      />
              ...
              <div className="max-w-4xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  {relatedPosts.map((post) => (
                    <Link key={post.id as string} href={`/blog/${post.slug as string}`} className="block group">
