...
/**
 * Soft delete (move to trash) one or more partner companies.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { failed, errors } = await deleteContentMany('partner-companies', idsToDelete, {
      deletedBy: auth.user.id,
    })

    if (failed.length > 0) {
      console.error('Error soft deleting partner companies:', errors)
      return NextResponse.json({
        error: `Failed to delete some partner companies: ${failed.join(', ')}`
      }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Unexpected error in partner-companies delete:', error)
