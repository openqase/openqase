...
/**
 * Soft delete (move to trash) one or more quantum software.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { failed, errors } = await deleteContentMany('quantum-software', idsToDelete, {
      deletedBy: auth.user.id,
    })

    if (failed.length > 0) {
      console.error('Error soft deleting quantum software:', errors)
      return NextResponse.json({
        error: `Failed to delete some quantum software: ${failed.join(', ')}`
      }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Unexpected error in quantum-software delete:', error)
