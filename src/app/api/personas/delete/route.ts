...
/**
 * Soft delete (move to trash) one or more personas.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { failed, errors } = await deleteContentMany('personas', idsToDelete, {
      deletedBy: auth.user.id,
    })

    if (failed.length > 0) {
      console.error('Error soft deleting personas:', errors)
      return NextResponse.json({
        error: `Failed to delete some personas: ${failed.join(', ')}`
      }, { status: 500 })
    }
