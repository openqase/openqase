...
/**
 * Soft delete (move to trash) one or more quantum hardware.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { failed, errors } = await deleteContentMany('quantum-hardware', idsToDelete, {
      deletedBy: auth.user.id,
    })

    if (failed.length > 0) {
      console.error('Error soft deleting quantum hardware:', errors)
      return NextResponse.json({
        error: `Failed to delete some quantum hardware: ${failed.join(', ')}`
      }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Unexpected error in quantum-hardware delete:', error)
