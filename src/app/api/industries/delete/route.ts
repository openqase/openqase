...
/**
 * Soft delete (move to trash) one or more industries.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { failed, errors } = await deleteContentMany('industries', idsToDelete, {
      deletedBy: auth.user.id,
    })

    if (failed.length > 0) {
      console.error('Error soft deleting industries:', errors)
      return NextResponse.json({
        error: `Failed to delete some industries: ${failed.join(', ')}`
      }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Unexpected error in industries delete:', error)
