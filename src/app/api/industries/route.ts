...
export async function GET(request: NextRequest) {
  try {
    ...
    if (slug) {
      const data = await fetchContentBySlug('industries', slug)
    ...
    const { items, total } = await listContent('industries', { page, pageSize })
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = await deleteContent('industries', id, { deletedBy: auth.user.id })
    if (!result.success) return NextResponse.json({ error: 'Failed to delete industry' }, { status: 500 })
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const result = published
      ? await publishContent('industries', id)
      : await unpublishContent('industries', id)
