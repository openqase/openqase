...
/**
 * Soft delete (move to trash) one or more quantum companies.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { failed, errors } = await deleteContentMany('quantum-companies', idsToDelete, {
      deletedBy: auth.user.id,
    })

    if (failed.length > 0) {
      console.error('Error soft deleting quantum companies:', errors)
      return NextResponse.json({
        error: `Failed to delete some quantum companies: ${failed.join(', ')}`
      }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Unexpected error in quantum-companies delete:', error)
